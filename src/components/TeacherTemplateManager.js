...
function TeacherTemplateManager() {
  ...
  const filteredSharedTemplates = React.useMemo(() => {
    if (!sharedTemplatesData || !user?.user_id) return [];
    return sharedTemplatesData.filter(t => t.creator_id !== user.user_id);
            ...
            <Tab eventKey="shared-templates" title="Shared Templates">
              ...
              {!isLoadingSharedTemplates && !errorSharedTemplates && (
                filteredSharedTemplates && filteredSharedTemplates.length > 0 ? (
                  ...
                  <Alert variant="info">No templates are currently shared by other users.</Alert>
                )
              )}
            </Tab>
          </Tabs>
